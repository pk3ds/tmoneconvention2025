//resources/js/Pages/Polls/Index.vue
<script setup>
...
const search = () => {
    router.get(route("polls.index", { search: form.search }));
};

const destroy = (poll) => {
    router.delete(route("polls.destroy", poll));
};

const restore = (id) => {
    router.put(route("polls.restore", id));
};

const hasVoted = (poll) => {
    return (
        poll.votes?.some((vote) => vote.user_id === page.props.auth.user.id) ??
        false
    );
};
</script>
                        ...
                        <div class="sm:flex sm:items-center">
                            ...
                            <div class="mt-4 sm:ml-16 sm:mt-0">
                                <Link
                                    ...
                                    "
                                >
                                    <PrimaryButton>Add poll</PrimaryButton>
                                </Link>
                            </div>
                        </div>

                        <!-- Mobile-friendly Poll List -->
                        <div class="mt-8">
                            <h1
                                v-if="!polls.data?.length"
                                class="text-base text-gray-900"
                            >
                                No polls to display
                            </h1>
                            <div v-else class="space-y-4">
                                <!-- Poll Card for Mobile -->
                                <div
