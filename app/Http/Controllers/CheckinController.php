...
class CheckinController extends Controller
{
    ...
    public function store($uuid)
    {
        ...
        // Convert session dates to Carbon instances for proper comparison
        $sessionStart = Carbon::parse($session->start_at);
        $sessionEnd = Carbon::parse($session->end_at);

        // Calculate points based on timing
        $points = 500; // Default points (during session)
        if ($now < $sessionStart) {
            $points = 1000; // Early check-in bonus
        } elseif ($now > $sessionEnd) {
            $points = -500; // Late check-in penalty
        }
                    ...
                    if (!$member->sessions()->where('session_id', $session->id)->exists()) {
                        ...
                        activity()
                            ...
                            ->withProperties([
                                ...
                                'timing' => $now < $sessionStart ? 'early' : ($now > $sessionEnd ? 'late' : 'on-time')
                            ])
                            ->event($session->name)
